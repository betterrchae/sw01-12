...
public class Game {
    ...
    public boolean moveHorse(Horse horse, YutResult result) {
        ...
        if (nextSpot != null && nextSpot.isFinish()) {
            ...
            if (horse.isInGroup()) {
                for (Horse h : horse.getGroup().getHorses()) {
                    h.setFinished(true);
                    board.updateHorsePosition(h); // 보드에서 자동 제거
                }
            } else {
            ...
            // 완주 후 턴 체크 (모두 완주했으면 게임 종료)
            if (currentPlayer.isAllHorsesFinished()) {
                endGame();
            }
            if (currentResults.isEmpty() && !canThrowAgain) {
                nextTurn();
            }
            return true;
        }
        ...
        List<Horse> enemyHorses = horsesAtSpot.stream().filter(h -> h.getOwner() != currentPlayer && !nextSpot.isFinish()).collect(Collectors.toList());
        ...
        if (!enemyHorses.isEmpty()) {
            ...
            List<Horse> friendlyHorses = horsesAtSpot.stream().filter(h -> h.getOwner() == currentPlayer && h != horse).collect(Collectors.toList());
